...
def get(session, data):
    ...
    try:
        if data.get('post_id'):
            ...
            errors.append('Post id: {post_id} not found'.fomrat(**data))
            post = None
    ...
    try:            
        ...
        if len(data['tags']) >=1:
            for tag_name in data['tags']:
                tag_result = tag.get(session=session, data={'name': tag_name})
        ...
        if data.get('owner_user_id'):
            owner_result = user.get(session=session, data={'user_id': data['owner_user_id']})
            if not owner_result.get('errors'):
                owner_obj = owner_result['result']
            else:
                error.extend(owner_result['errors'])
        
        # EDITOR USER
        editor_obj = None
        if data.get('last_editor_user_id'):
            editor_result = user.get(session=session, data={'user_id': data['last_editor_user_id']})
            if not editor_result.get('errors'):
                editor_obj = editor_result['result']
            else:
                error.extend(editor_result['errors'])

        # Check if question exist
        question_exists = False
        question_obj = None
        if data.get('post_type_id', 0) == 2 and data.get('parent_id'):
            question_result = get(session=session, data={'post_id': data['parent_id']})
