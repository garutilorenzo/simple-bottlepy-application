...
import utils, load_config, db_redis, db_api
...
@view('users')
def get_users(page_nr=1):
    current_page = utils.get_first_level_url(request)

    users = []
    users_result = db_redis.user.get_all(limit=main_config['default_result_limit'], offset=page_nr)
    count = 0
...
@view('user')
def get_user(id=0, user_name=''):
    current_page = utils.get_first_level_url(request)

    user_result = db_redis.user.get(filters={'id': id})
...
@view('tags')
def get_tags(page_nr=1):
    current_page = utils.get_first_level_url(request)

    tags = []
    tags_result = db_redis.tag.get_all(limit=main_config['default_result_limit'], offset=page_nr)
    count = 0
    if not tags_result.get('errors'):
        tags = tags_result['result']
        count = tags_result['count']
...
@view('posts')
def post_search_posts(page_nr=1):
...
def get_posts(page_nr=1):
    ...
    posts = []
    count_all_posts = 0
    posts_result = db_redis.post.get_all_filtered(
        limit=main_config['default_result_limit'], 
        offset=page_nr,
        filters=filters,
        filters_like=filters_like,
...
@view('post')
def get_post(id=0, post_title=''):
    current_page = utils.get_first_level_url(request)

    posts_result = db_redis.post.get(filters={'id': id})
    if not posts_result.get('error'):
        post = posts_result['result'][0]
